...
export const HighDemandContentSection: React.FC<HighDemandContentSectionProps> = ({
  ...
  return (
    <div
      ...
      className="mt-8 rounded-2xl border border-slate-800 bg-slate-900/60 p-5 sm:p-6 shadow-sm backdrop-blur-sm relative overflow-hidden transition"
    >
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-5 border-b border-slate-800 relative z-10">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="flex h-6 w-6 items-center justify-center rounded-md bg-indigo-600/20 text-indigo-400">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            disabled={isLoading}
            className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-750 text-slate-300 hover:text-white border border-slate-700 text-xs font-medium transition flex items-center gap-1.5 cursor-pointer shadow-sm disabled:opacity-50"
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-medium transition flex items-center gap-1.5 shadow-sm active:scale-95 cursor-pointer"
      ...
      <div className="my-4 p-3 rounded-xl bg-slate-950/60 border border-slate-800 relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      ...
      {effectiveNiche === "podcast" && (
        <div className="mb-4 px-3 py-2 rounded-lg bg-slate-950/60 border border-slate-800 flex items-center justify-between text-xs text-slate-300 relative z-10">
          <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-medium uppercase tracking-wider text-slate-400 bg-slate-800 px-2 py-0.5 rounded">
            Filtered
          </span>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="mt-5 space-y-6 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 rounded-xl border border-slate-800 bg-slate-950/60 p-4">
            ...
            <div className="md:col-span-3 border-t md:border-t-0 md:border-l border-slate-800 pt-3 md:pt-0 md:pl-4">
              <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block mb-1">
                Niche Demand Index
              </span>
            ...
            <div className="md:col-span-3 border-t md:border-t-0 md:border-l border-slate-800 pt-3 md:pt-0 md:pl-4">
              ...
              <div className="flex items-center gap-1.5 mt-0.5 text-white font-semibold text-xs font-mono">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-3">
            <div className="flex items-center gap-1 rounded-xl bg-slate-950/80 p-1 border border-slate-800 text-xs font-medium">
              <button
                type="button"
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 pb-3 border-b border-slate-800/80">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="flex h-5 w-5 items-center justify-center rounded-md bg-slate-800 text-slate-300 text-[11px] font-bold">
                            #{index + 1}
                          </span>
                          <span className="text-xs font-semibold text-white tracking-wide">
                            {opp.topic}
                          </span>
                          <span className="text-[10px] font-medium text-slate-400 bg-slate-800/80 px-2 py-0.5 rounded border border-slate-750">
                            {opp.nicheCategory}
                          </span>
                        </div>

                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-medium px-2 py-0.5 rounded border border-slate-750 bg-slate-900 text-slate-300 flex items-center gap-1 font-mono">
                            {opp.demandLevel} • {opp.demandScore}/100
                          </span>

                          <span className="text-[10px] font-medium px-2 py-0.5 rounded bg-slate-900 text-slate-300 border border-slate-750 flex items-center gap-1 font-mono">
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mt-4 text-xs">
                        ...
                        <div className="lg:col-span-7 space-y-3">
                          <div>
                            ...
                            <p className="text-slate-300 leading-relaxed text-xs bg-slate-900/60 p-2.5 rounded-xl border border-slate-800">
                              {opp.whyDemandIsHigh}
                            </p>
                          </div>
                          ...
                          {/* Projected Impact */}
                          <div className="flex items-start gap-2 text-[11px] text-slate-300 bg-slate-900/60 px-3 py-2 rounded-xl border border-slate-800">
                              ...
                              {opp.suggestedTitles.map((st, sIdx) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className="w-full text-left p-2 rounded-lg bg-slate-900 hover:bg-slate-850 border border-slate-800 hover:border-slate-700 transition group cursor-pointer flex items-center justify-between gap-2"
                                    title="Click to copy title"
                                  >
                                    <span className="text-slate-200 text-xs group-hover:text-white line-clamp-1">
                                      "{st}"
                                    </span>
                ...
                <div className="mt-5 rounded-xl border border-slate-800 bg-slate-950/60 p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                    {demandData.productionActionPlan.map((step, idx) => (
                      <div
                        key={idx}
                        className="rounded-lg bg-slate-900/80 p-3 border border-slate-800 text-slate-300"
                      >
                        <span className="inline-flex h-5 w-5 items-center justify-center rounded bg-slate-800 text-slate-300 font-bold text-[10px] mr-1.5 font-mono">
                          {idx + 1}
                        </span>
                        <span>{step}</span>
                      </div>
          ...
          {activeTab === "formats" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {demandData.highestRatedFormats.map((fmt, fIdx) => (
                <div
                  key={fIdx}
                  className="rounded-xl border border-slate-800 bg-slate-950/60 p-4 relative overflow-hidden"
                >
                  <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                    ...
                    <span className="text-[10px] font-medium px-2 py-0.5 rounded bg-slate-900 text-slate-300 border border-slate-800 font-mono">
                  ...
                  <div className="mt-2 text-xs text-slate-300 bg-slate-900/60 p-2.5 rounded-lg border border-slate-800 leading-relaxed">
                    <span className="text-[10px] font-medium text-slate-400 block mb-0.5">
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {demandData.trendingViewerQueries.map((query, qIdx) => (
                  <div
                    key={qIdx}
                    className="flex items-start justify-between gap-2 p-3 rounded-xl bg-slate-900/80 border border-slate-800 text-xs text-slate-200"
                  >
                    <div className="flex items-center gap-2">
